...
public struct Session: Sendable {
        ...
        -> RefreshSessionResult
    {
        ...
        do {
            ...
            // A terminal refresh failure surfaces as the OAuth `invalid_grant`
            // error code (the refresh token is expired, revoked, or reused past
            // the grace window). The WorkOS token endpoint returns it at HTTP
            // 400, so detection keys off the error code rather than a specific
            // status. Any failure without that code (rate limits, 5xx, network)
            // stays the generic `refresh_failed`, so the caller can retry
