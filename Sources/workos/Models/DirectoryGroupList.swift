...
public struct DirectoryGroupList: Codable, Sendable, Equatable {
    /// Indicates this is a list response.
    public let object: String
    /// The list of records for the current page.
    public let data: [DirectoryGroup]
    /// Pagination cursors for navigating between pages of results.
    public let listMetadata: DirectoryGroupListListMetadata

    public init(
        object: String,
        data: [DirectoryGroup],
        listMetadata: DirectoryGroupListListMetadata
    ) {
        self.object = object
        self.data = data
